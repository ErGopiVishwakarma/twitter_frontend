...
const Post = ({ el }) => {
  ...
  let a = postDate.toLocaleString('default', { month: 'long' }).substring(0,3)
    ...
    return (
        <Flex gap="20px" w="100%" px='10px' py='15px' _hover={{backgroundColor:'gray.100'}} boxShadow={'rgba(0, 0, 0, 0.05) 0px 0px 0px 1px'}>
        <Box>
        ...
        <Flex w='100%' direction={'column'} alignItems={'flex-start'} gap="8px">
            <Flex justifyContent={'space-between'} w="100%" >
                <NavLink to={`profile/${el?.postedBy?._id}`}>
                    <Flex alignItems={'center'} gap='3px'>
                        <Text fontSize={'15px'} fontWeight={'bold'} color="gray.700">{el?.postedBy.name}</Text>
                        <Text fontSize={'14px'} fontWeight={'bold'} color="gray.500">@{el?.postedBy.name.toLowerCase().replaceAll(" ","")}.</Text>
                        {
                            second < 24 ?<Text pl='15px' fontSize={'14px'} fontWeight={'bold'} color="gray.600">{second}h ago</Text>:
                            <Text pl='15px' fontSize={'14px'} fontWeight={'bold'} color="gray.600">{`${a}${b}`}</Text>
                        }
                ...
                <Menu boxShadow={'rgba(0, 0, 0, 0.3) 0px 19px 38px, rgba(0, 0, 0, 0.22) 0px 15px 12px'}>
                    <MenuButton>
            ...
            <Box borderRadius={{ base: "15px", md: '17px' }} w="100%">
                <Image src={el?.picture} borderRadius={{ base: "15px", md: '17px' }}
