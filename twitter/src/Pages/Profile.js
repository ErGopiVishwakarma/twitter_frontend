...
const Profile = () => {
  ...
  const userProfile = async () => {
    try {
      ...
      const { data } = await axios.get(`https://curious-plum-stole.cyclic.app/user/profile/${userId}`, config)
  ...
  const followUser = async () => {
    try {
      ...
      const { data } = await axios.put(`https://curious-plum-stole.cyclic.app/user/follow`, {
  ...
  const unfollowUser = async () => {
    try {
      ...
      const { data } = await axios.put(`https://curious-plum-stole.cyclic.app/user/unfollow`, {
  ...
  const changeBg = async (parameter) => {
    try {
      ...
      const { data } = await axios.put(`https://curious-plum-stole.cyclic.app/user/bgpicchange`, {
        bgpic: parameter
      }, config)
      setAgainRender(prev => !prev)
  ...
  return (
    <Flex w='100%'>
    {/* this is left part  */}
    <Box w={{ base: '100%', sm: '100%', md: '100%', lg: '65%' }} px="20px"
      overflowY={'auto'} h="100vh" position={'relative'} pt='10px'
      css={{
        '&::-webkit-scrollbar': {
          width: '4px',
        },
    ...
    >
      ...
      {/* ===================setting up background and user profile pare from here to */}
      {
        !profile?.backgroundpic ? <Box h="250px" w='100%' bg="gray.300" pt='60px' position={'relative'}>
          {
            profile?._id === token._id ? <Box p='15px' _hover={{ backgroundColor: 'red.100' }}
          ...
          {
            profile?._id === token._id ? <Box p='15px' bg='red.100' borderRadius={'50%'} position={'absolute'}
      ...
      <Flex justifyContent={'flex-end'} pt="15px">
        {
          profile?._id === token._id ? <Button borderRadius={'50px'} border={'1px solid black'}
    ...
    <Box w='35%' display={{ base: 'none', sm: 'none', md: 'none', lg: 'block' }}>
      <Flex w='100%' position={'relative'} zIndex={2} direction={'column'}
        ml='15px' gap='20px' h='100vh' overflowY={'scroll'} pt='10px'
      ...
      >
        ...
        <Flex direction={'column'} justifyContent={'flex-start'} bg='gray.100'>
          <Heading fontSize='22px' color='#0F1419 ' fontFamily={'revert'}
            p='15px'>What is happening</Heading>
          <Flex direction={'column'} >
            {
              trendingArr.map((el, ind) => (
                <Flex direction={'column'} _hover={{ backgroundColor: 'gray.200' }}
                  px='15px' py='10px' w='100%' key={ind}  >
